...
def train(config):
    ...
    dataloader = torch.utils.data.DataLoader(dataset,
                                             ...
                                             shuffle=True, drop_last=True, num_workers=32, pin_memory=True)
...
def _get_optimizer(config, net):
    ...
    params = None
    base_params = list(
        map(id, net.backbone.parameters())      # id　返回对象的内存地址
    )
    logits_params = filter(lambda p: id(p) not in base_params, net.parameters())  # 过滤 backbone 的参数，留下剩余参数
